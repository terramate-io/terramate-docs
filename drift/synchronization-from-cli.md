...
You can synchronize deployments using [`terramate run`](/cli/reference/cmdline/run.md) or reduce the overhead on the caller side by using [Terramate Scripts](/orchestration/scripts.md), such as `terramate script run`, which triggers deployment sync automatically.
