...
It is recommended to use a lowercase [UUIDv4](<https://en.wikipedia.org/wiki/Universally_unique_identifier#:~:text=Version%204%20(random)%5Bedit%5D>) as stack ID as this is the default when a stack is created using the [`terramate create`](/cli/reference/cmdline/create.md) command.

When stacks are [cloned](/cli/reference/cmdline/experimental/experimental-clone.md) a new UUIDv4 is generated for cloned stacks.
...
- A [tag filter](/orchestration/.md#filter-by-tags) to match multiple stacks.
...
See the [orchestration docs](/orchestration/.md#order-of-execution) for details.
...
The list of files that must be watched for changes in the [change detection](../orchestration/change-detection/index.md).
