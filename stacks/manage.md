...
filter (valid statuses are documented on the [trigger page](/cli/reference/cmdline/trigger.md)).
...
The [`trigger`](/cli/reference/cmdline/trigger.md) command forcibly marks a stack as "changed" even if it doesn't contain any code
changes according to the [change detection](../orchestration/change-detection/index.md).

```sh
terramate trigger <stack>
```

For details, please see the [`trigger`](/cli/reference/cmdline/trigger.md) documentation.
